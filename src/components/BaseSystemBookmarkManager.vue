  ...
  useKeycapDrag({
    ...
    swapData: async (source, target) => {
      ...
      if (targetData) {
        ...
        await swapBookmarksInSourceFolder(sourceFolderPath, source, target)
      } else {
        // 目标未绑定：移动到目标位置
        ...
        await removeBookmarkFromSourceFolder(sourceFolderPath, source)
      }
    },
...
const onSelectBookmark = async (payload: { title: string; url?: string }) => {
  ...
  try {
    ...
    if (oldEntry?.url) {
      ...
      await addBookmarkToSourceFolder(sourceFolderPath, code, payload.url, name)
    }

    // 3. 重新解析书签树
    await getSystemBookmarkForKeyboard()
  } catch (e) {
    console.error('[BaseSystemBookmarkManager] bind failed:', e)
  } finally {
...
const syncBookmark = async () => {
  ...
  try {
    if (!url) {
      ...
      // URL 存在（更新）
      await updateBookmarkInSourceFolder(sourceFolderPath, code, url, name)
    } else {
      // URL 从空变有（新建）
    ...
    console.error('[BaseSystemBookmarkManager] sync bookmark failed:', e)
  } finally {
    isSyncing = false
  }
}
...
const onUnbind = async () => {
  ...
  try {
    ...
    console.error('[BaseSystemBookmarkManager] unbind failed:', e)
  } finally {
    originUrl = ''
...
<template>
  ...
  <div class="base-system-bookmark-manager">
    <!-- 键盘预览 -->
    <div
      class="system-bookmark-keyboard"
      :style="cssVars"
    >
      <KeyboardLayout
        unit="px"
      ...
      >
        <template #keycap="{ code }">
          <div
            class="system-keycap-drag-wrap"
            :class="{
              'system-keycap-drag-wrap--drag-target': isDropTarget(code),
            }"
    ...
    <!-- 表单区域 -->
    <div class="system-bookmark-form">
...
.system-bookmark-keyboard {
  display: flex;
  justify-content: center;

  .system-keycap-drag-wrap {
    width: 100%;
    height: 100%;
  ...
  .system-keycap-drag-wrap--drag-target {
