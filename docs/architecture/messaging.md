...
│                      │    │                          │
│  • index.ts: 按键采集 │    │  • task/ keydownTaskMap│
│  • scroll.ts: 滚动    │    │  • shortcut-executor.ts  │
...
- `shortcut-executor.ts` 注册统一 keydown task（书签 + 命令，命令优先）
...
书签和命令快捷键合并到 `shortcut-executor.ts` 的 `globalShortcutTask` 单一函数内。同时匹配书签和命令，冲突时命令优先（与 CS 端行为一致）。
